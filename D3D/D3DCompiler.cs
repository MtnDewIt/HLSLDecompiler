...
namespace HLSLDecompiler.D3D
{
    public static class D3DCompiler
    {
        ...
        public static byte[] GenerateByteCode(string template, IEnumerable<D3D.SHADER_MACRO> macros, string entry, string version)
        {
            ...
            D3DIncludeManager include = new D3DIncludeManager(template.Replace(fileName, ""));

            string shader_source = include.ReadResource(fileName);

            D3DCOMPILE flags = 0;

            //flags |= D3DCOMPILE.D3DCOMPILE_WARNINGS_ARE_ERRORS;
            //flags |= D3DCOMPILE.D3DCOMPILE_SKIP_VALIDATION;
            //flags |= D3DCOMPILE.D3DCOMPILE_DEBUG;
            flags |= D3DCOMPILE.D3DCOMPILE_OPTIMIZATION_LEVEL3; // if can't get shader to compile 1-1 add or remove this line
        ...
        public static byte[] Compile(string Source, string Entrypoint, string Target, D3D.SHADER_MACRO[] Defines = null, D3DCOMPILE Flags1 = 0, uint Flags2 = 0, string SourceName = null, D3DInclude include = null)
        {
            // Macros should never be duplicated
